...
def aplicar_formato_final_excel(writer, df):
    ...
    # Lógica para recorrer cada bloque de sitio
    start_row = 2
    # Usamos el DataFrame ordenado que se pasó a la función
...
def procesar_archivo_de_datos(ruta_archivo):
    ...
    try:
        ...
        # Ordenar por 'Nombre del sitio' de forma descendente
        ...
        with pd.ExcelWriter(ruta_excel_salida, engine='openpyxl') as writer:
            # Pasamos el DataFrame ya ordenado
            df_final.to_excel(writer, index=False, sheet_name='Vertices')
            # Y se lo pasamos también a la función de formato
